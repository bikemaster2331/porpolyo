...
const ACHIEVEMENTS_DATA = [
    {
        ...
        period: "2022 – 2026",
        desc: "Relevant Coursework: Computer Networks & TCP/IP, Data Structures & Algorithms, Object-Oriented Programming, Machine Learning, Edge AI, Embedded Systems, Digital Signal Processing.",
        hoverText: "THE GRIND NEVER STOPS.",
        rotate: "rotate-[1deg]",
        attachment: { type: 'clip', position: '-top-4 left-8 md:left-12' }
    },
    {
        title: "1st Place Winner (Team Leader)",
        period: "2026",
        desc: "Regional ICpEP Robothon Challenge.",
        hoverText: "HARD WORK PAYS OFF.",
        rotate: "rotate-[-1deg]",
...
export default function Work() {
    ...
    const handleHoverLeave = () => {
        hoverTimeoutRef.current = setTimeout(() => setIsHovering(false), 50);
    };
    ...
    const ContentSections = ({ isMobile }: { isMobile?: boolean }) => (
        <>
            <div className={`${isMobile ? 'mb-10 md:mb-16' : 'mb-8'} flex flex-col relative items-start w-full`}>
                {isMobile && (
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-14 ${isMobile ? 'mb-16 mt-4 md:mt-12' : 'mb-12 mt-4'} relative w-full`}>
                ...
                <div className="flex flex-col gap-6 md:gap-8 w-full">
                    ...
                    <div className="flex flex-col gap-8 relative z-10">
                        {ACHIEVEMENTS_DATA.map((ach) => (
                            <div
                                key={ach.title}
                                className={`relative bg-[#F4EFEA] p-4 md:p-8 border-4 border-black shadow-[6px_6px_0_rgba(0,0,0,1)] md:shadow-[8px_8px_0_rgba(0,0,0,1)] ${ach.rotate} hover:rotate-0 hover:scale-[1.02] hover:shadow-[12px_12px_0_rgba(0,0,0,1)] hover:-translate-y-1 hover:-translate-x-1 transition-all duration-300 cursor-default`}
                                onMouseEnter={() => handleHoverEnter(ach.hoverText)}
                                onMouseLeave={handleHoverLeave}
                            >
                                <Attachment {...ach.attachment} />
                                <div className="font-mono text-[9px] md:text-[10px] font-bold text-black/60 uppercase tracking-widest border-b-2 border-black/20 pb-2 mb-4 md:mb-4">{ach.period}</div>
                                <h4 className="text-lg md:text-2xl font-black font-sans text-black uppercase tracking-tighter mb-2 md:mb-2 pr-4">{ach.title}</h4>
                                <p className="text-[13px] md:text-[14px] text-black font-mono font-bold leading-relaxed pt-2 md:pt-2 mt-2">
                                    {ach.desc}
                                </p>
                            </div>
                        ))}
            ...
            {/* LINKS */}
            <div className={`flex flex-wrap gap-4 md:gap-6 w-full mt-4 md:mt-4 pl-1 md:pl-2 ${isMobile ? 'pb-8' : 'pb-4'} relative z-20`}>
                <a href="mailto:tanlanuzga@gmail.com" className="flex items-center justify-center p-4 sm:p-4 md:p-5 bg-white border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_rgba(0,0,0,1)] hover:shadow-[0px_0px_0_rgba(0,0,0,1)] hover:translate-y-[4px] hover:translate-x-[4px] md:hover:translate-y-[6px] md:hover:translate-x-[6px] transition-all duration-150 group">
                ...
                <a href="https://www.linkedin.com/in/marthan-lanuzga-01b8b5287/" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-4 sm:p-4 md:p-5 bg-[#0077B5] border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_rgba(0,0,0,1)] hover:shadow-[0px_0px_0_rgba(0,0,0,1)] hover:translate-y-[4px] hover:translate-x-[4px] md:hover:translate-y-[6px] md:hover:translate-x-[6px] transition-all duration-150 group">
                ...
                <a href="https://github.com/bikemaster2331" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-4 sm:p-4 md:p-5 bg-zinc-800 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_rgba(0,0,0,1)] hover:shadow-[0px_0px_0_rgba(0,0,0,1)] hover:translate-y-[4px] hover:translate-x-[4px] md:hover:translate-y-[6px] md:hover:translate-x-[6px] transition-all duration-150 group">
                    <svg viewBox="0 0 128 128" className="w-6 h-6 md:w-6 md:h-6 fill-white">
    ...
    return (
        <div className="flex flex-col items-center min-h-screen relative text-white pb-24 md:pb-0 px-4 sm:px-6 overflow-x-clip">
            ...
            <div className="max-w-[1600px] mx-auto w-full flex-grow md:pt-36 relative z-10 px-2 sm:px-4 md:px-12 flex flex-col lg:flex-row">
                ...
                <div className="hidden lg:flex flex-col gap-3 absolute top-36 left-12 z-30 pointer-events-none">
                    <h1 className="text-5xl font-black tracking-tighter uppercase drop-shadow-[4px_4px_0_rgba(0,0,0,1)] text-white font-sans flex items-baseline">
                        ./WORK
                    </h1>
                </div>

                {/* ── MOBILE VIEW: Exact restoration from first block ── */}
                    ...
                    <div className="w-full max-w-[380px] h-[240px] relative">
                        <div className="absolute -top-6 left-0 z-30 pointer-events-auto w-[220px] -rotate-3">
                            <div className={`transition-all duration-500 transform ${modelReply ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                                <div className="relative bg-[#F5E6D3] text-black p-3 rounded-2xl border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] transition-all duration-300">
                                    <p className="font-mono text-[10px] leading-snug uppercase font-bold tracking-tight text-center w-full break-words relative z-10">
                                        <span className="invisible select-none" aria-hidden="true">{modelReply || "AWAITING_INPUT"}</span>
                                        <span className="absolute top-0 left-0 w-full">{displayedReply}</span>
                                    </p>
                                    <div className="absolute -bottom-3.5 left-6 w-4 h-4 bg-[#F5E6D3] border-2 border-black rounded-full shadow-[2px_2px_0_rgba(0,0,0,1)]" />
                                    <div className="absolute -bottom-6 left-10 w-2.5 h-2.5 bg-[#F5E6D3] border-2 border-black rounded-full shadow-[1px_1px_0_rgba(0,0,0,1)]" />
                                </div>
                            </div>
                        </div>
                        ...
                        {hasMounted && (
                            <model-viewer
                                ...
                                {...({ reveal: 'auto', 'draco-decoder-config': JSON.stringify({ type: 'js' }) } as any)}
                            >
                ...
                <div className="w-full lg:hidden flex flex-col relative z-10">
                    <ContentSections isMobile={true} />
                </div>

                {/* ── DESKTOP VIEW: Sliding Laptop Interface ── */}
                <div className="hidden lg:flex w-full items-start overflow-hidden">
                    
                    {/* ① Left spacer */}
                    <div className="shrink-0 overflow-hidden transition-[width] duration-700 ease-in-out" style={{ width: showComputer ? '0%' : '20%' }} />

                    {/* ② GLB column (Clickable Model) */}
                    <div className="shrink-0 transition-[width] duration-700 ease-in-out relative" style={{ width: showComputer ? '40%' : '60%' }}>
                        <div className="sticky top-40 w-full">
                            <div className="w-full h-[650px] relative drop-shadow-[0_4px_24px_rgba(255,255,255,0.4)]">
                                
                                <div className="absolute top-[20%] left-[20%] z-30 pointer-events-none max-w-[240px] -rotate-2">
                                    <div className={`transition-all duration-500 transform ${modelReply ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                                        <div className="relative bg-[#F5E6D3] text-black px-4 py-3 rounded-2xl border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)]">
                                            ...
                                            <div className="absolute -bottom-4.5 right-8 w-5 h-5 bg-[#F5E6D3] border-2 border-black rounded-full shadow-[2px_2px_0_rgba(0,0,0,1)]" />
                                            <div className="absolute -bottom-7.5 right-6.5 w-3 h-3 bg-[#F5E6D3] border-2 border-black rounded-full shadow-[1px_1px_0_rgba(0,0,0,1)]" />
                                        </div>
                                    </div>
                                </div>

                                <div className="w-full h-full absolute inset-0 cursor-pointer" onClick={() => setShowComputer(v => !v)}>
                                    {hasMounted && (
                                        <model-viewer
                                            src="/models/study.glb"
                                            autoplay
                    ...
                    <div
                        ...
                        style={{
                            width: showComputer ? '60%' : '0%',
                            opacity: showComputer ? 1 : 0,
                            transition: showComputer
                                // OPENING: Width expands immediately (700ms). Opacity waits 1500ms, then fades in over 500ms smoothly.
                                ? 'width 700ms ease-in-out, opacity 500ms ease-in-out 1000ms'
                                
                                // CLOSING: Opacity fades out immediately (300ms). Width waits 300ms, then shrinks over 700ms.
                                : 'opacity 300ms ease-in-out, width 700ms ease-in-out 300ms'
                        }}
                    >
                        <div className="pl-6 pt-0">
                            <ComputerScreen onClose={() => setShowComputer(false)}>
                                <ContentSections isMobile={false} />
                            </ComputerScreen>
                        </div>
                    </div>
                </div>
...
// ── Styled Laptop Wrapper ──
function ComputerScreen({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
    return (
        <div className="w-full flex flex-col items-center">
            <div className="w-full h-[70vh] border-[12px] border-[#111] bg-[#1A1A1A] rounded-t-2xl rounded-b-sm relative flex flex-col shadow-2xl overflow-hidden">
                <div className="absolute top-[-8px] left-1/2 -translate-x-1/2 w-3 h-3 bg-black rounded-full z-30 flex items-center justify-center border border-white/5">
                    <div className="w-1 h-1 bg-blue-900/40 rounded-full" />
                </div>
                <div className="flex items-center justify-between px-3 py-2 bg-[#1a1a1a] shrink-0 z-20 border-b border-white/5">
                    <span className="font-mono text-[9px] text-white/30 uppercase tracking-widest">marthan.exe</span>
                    <button onClick={onClose} className="font-mono text-[10px] text-white/30 hover:text-white/80 transition-colors uppercase tracking-widest">
                        ✕
                    </button>
                </div>
                <div className="flex-1 overflow-y-auto p-4 md:p-6 z-20 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/40">
                    {children}
                </div>
                <div className="absolute inset-0 pointer-events-none z-10 bg-gradient-to-tr from-transparent via-transparent to-white/[0.03]" />
            </div>
            <div className="w-[102%] h-4 bg-gradient-to-b from-[#333] to-[#111] rounded-b-xl shadow-[0_15px_25px_rgba(0,0,0,0.8)] relative flex justify-center">
                <div className="w-1/5 h-1 bg-black/40 rounded-b-md absolute top-0" />
            </div>
        </div>
    );
}