...
const TechIcons: Record<string, React.ReactNode> = {
  ...
  React: (
    <svg viewBox="-11.5 -10.23174 23 20.46348" fill="none" stroke="#61DAFB" strokeWidth="1" className="w-full h-full">
  ...
  "Next.js": (
    <svg viewBox="0 0 128 128" fill="currentColor" className="w-full h-full text-white">
      <path d="M64 0C28.7 0 0 28.7 0 64s28.7 64 64 64c11.2 0 21.7-2.9 30.8-7.9L48.4 55.4v31.7h-9.5V40.9h9.5l45.4 59c7.9-9.1 12.7-21 12.7-33.9C128 28.7 99.3 0 64 0zM118.5 64c0 11.2-3.6 21.6-9.7 30.1L68.4 40.9h12.3l37.8 51.5c.3-.8.6-1.6.8-2.4V40.9h9.5v28.2c-.4-1.7-.3-3.4-.3-5.1z" />
    </svg>
  ),
  Algorithms: (
    <svg viewBox="0 0 24 24" fill="none" stroke="#A855F7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full">
      <polyline points="16 18 22 12 16 6" />
      <polyline points="8 6 2 12 8 18" />
    </svg>
  ),
  AST: (
    <svg viewBox="0 0 24 24" fill="none" stroke="#3B82F6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full">
      <circle cx="12" cy="5" r="3" />
      <circle cx="5" cy="19" r="3" />
      <circle cx="19" cy="19" r="3" />
      <line x1="12" y1="8" x2="7" y2="16.5" />
      <line x1="12" y1="8" x2="17" y2="16.5" />
    </svg>
  ),
  RAG: (
    <svg viewBox="0 0 24 24" fill="none" stroke="#F43F5E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full">
...
const TechBadge = ({ name }: { name: string }) => {
  ...
  return (
    <div className="flex items-center gap-1.5 px-1.5 md:px-2 py-1 rounded-md bg-white/[0.03] border border-white/5 hover:bg-white/10 transition-all group/badge">
      <div className="w-3.5 h-3.5 opacity-70 group-hover/badge:opacity-100 transition-opacity">
        {icon}
      </div>
      <span className="hidden md:block text-[10px] font-mono text-zinc-500 group-hover/badge:text-white transition-colors leading-none tracking-tight">
        {name}
      </span>
    </div>
  );
...
export default function Projects() {
  ...
  // Desktop detection for hover effects
  useEffect(() => {
    const checkViewport = () => {
  ...
  const defaultDesc = "Get to know me more through my projects! I call this page the \"vault\". Click the cards for the repo!";
  ...
  useEffect(() => {
    const targetDesc = hoveredIndex !== null ? projectData[hoveredIndex].longDesc : defaultDesc;
    const targetSuffix = hoveredIndex !== null ? `/${projectData[hoveredIndex].name}` : "";
    ...
    const typeDesc = () => {
      if (descIdx < targetDesc.length) {
        ...
        descIdx++;
        sequenceTimeout = setTimeout(typeDesc, 12);
      }
    };
  ...
  return (
    <div className="flex flex-col items-center min-h-screen px-6 bg-black text-white pb-24 md:pb-0">
      <div className="w-full order-first md:order-last">
        <Footer />
      </div>
      <div className="max-w-5xl w-full flex-grow pt-4 md:pt-36">

        <div className="md:mb-16 mb-4 flex flex-col gap-3 min-h-[160px]">
          <h1 className="text-5xl font-satoshi font-bold tracking-tighter">
            . /projects{displayedSuffix}
          </h1>
          <p className="text-zinc-600 font-mono md:text-[14px] text-[12px] max-w-xl leading-relaxed mt-4 h-12">
            {displayedDesc}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 pb-6">
          {projectData.map((project, i) => (
            <a
              ...
              className={`group relative p-5 md:p-7 h-full flex flex-col rounded-xl bg-zinc-900/40 hover:bg-zinc-800/60 transition-all duration-700 cursor-pointer overflow-hidden border-2 border-zinc-800 hover:scale-[1.02] hover:!z-10 ${
                isDesktop && hoveredIndex !== null && hoveredIndex !== i 
                  ? 'blur-[4px] opacity-40' 
                  : 'blur-0 opacity-100'
              }`}
            >
              <h3 className="text-sm md:text-3xl font-bold lowercase font-mono tracking-tighter md:mb-9 mb-4 group-hover:text-white transition-colors duration-300">
                {project.name}
              </h3>
              ...
              <div className={`flex items-center justify-between mt-auto pt-4 border-t border-white/5 transition-all duration-700 ${
                isDesktop && hoveredIndex !== null && hoveredIndex !== i 
                  ? 'grayscale opacity-50' 
                  : 'grayscale-0 opacity-100'
              }`}>
                <div className="flex flex-nowrap md:flex-wrap gap-1 md:gap-2">
                  {project.tech.map(t => (
