...
const STACK_DATA = [
  {
    ...
    items: [
      ...
      { name: "C#", icon: "csharp", isCustom: true },
    ]
  },
  {
    category: "Frameworks",
  ...
  },
  {
    category: "AI/ML",
    items: [
      { name: "TF/Keras", icon: "tensorflow" },
      { name: "OpenCV", icon: "opencv" },
      { name: "Pandas", isCustom: true },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "ChromaDB", icon: "/images/chromadb.svg", isLocal: true },
    ]
  }
];
      ...
      <div className="max-w-7xl w-full flex-grow pt-4 md:pt-36">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:mb-20 mb-16">
          <div className="space-y-6 text-[12px] text-zinc-400 leading-relaxed font-sans">
            <p>
              I started coding during my first year of college, learned C++, C#, JS, coded on paper, made triangles using loops and conditions, and created CRUD systems. It was fun, but I didn't really take it seriously yet. <br />
              <br />
                ...
                <div className="grid grid-cols-2 md:grid-cols-1 gap-2 md:gap-3">
                  {col.items.map((item: any) => (
                    <div key={item.name} className="flex items-center gap-2 md:gap-3 border border-zinc-800 bg-zinc-800/40 p-2 md:p-3 md:hover:border-white md:hover:bg-white md:hover:text-black active:scale-95 active:bg-zinc-800 active:border-zinc-400 transition-all duration-200 cursor-default group/item">
                      {item.isCustom ? (
                        <div className="w-4 h-4 md:w-5 md:h-5 opacity-80 group-hover/item:opacity-100 transition-opacity shrink-0">
                          {item.name === "Pandas" ? (
                            ...
                            <svg viewBox="0 0 100 100" className="w-full h-full">
                              ...
                              <defs><clipPath id="a-csharp-about"><path fill="#fff" d="M6 0h88.889v100H6z"/></clipPath></defs>
                            </svg>
                          ) : null}
                        </div>
                      ) : (
                        <img 
                          src={item.isLocal ? item.icon : `https://cdn.simpleicons.org/${item.icon}`} 
                          alt={item.name} 
                          className="w-4 h-4 md:w-5 md:h-5 opacity-80 group-hover/item:opacity-100 transition-opacity shrink-0" 
                        />
                      )}
